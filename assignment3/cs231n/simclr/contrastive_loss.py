...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2 * N):
            if i != k:
                sum1 += (sim(z_k, out[i]) / tau).exp()
            if i != k + N:
                sum2 += (sim(z_k_N, out[i]) / tau).exp()
        
        total_loss -= ((sim(z_k, z_k_N) / tau).exp() / sum1).log()
        total_loss -= ((sim(z_k, z_k_N) / tau).exp() / sum2).log()
...
def compute_sim_matrix(out):
    ...
    sim_matrix = torch.mm(out, out.T) / torch.mm(torch.linalg.norm(out, dim = 1, keepdim = True), torch.linalg.norm(out.T, dim = 0, keepdim = True))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = (torch.cat((sim_pairs, sim_pairs)) / tau).exp()
