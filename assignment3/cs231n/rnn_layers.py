...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(range_num):
        h0, _ = rnn_step_forward(x[:, i, :], h0, Wx, Wh, b)
        h[:, i, :] = h0
        
    cache = (x, h, Wx, Wh, b, tmp)
...
def rnn_backward(dh, cache):
    ...
    # So dh are the gradients produced by the output? Maybe...
    
    x, h, Wx, Wh, b, h0 = cache
    
    range_num = x.shape[1] - 1
    ...
    for i in range(range_num, -1, -1):
        if i:
            tmp = rnn_step_backward(dh[:, i, :] + dh0, (x[:, i, :], h[:, i - 1, :], Wx, Wh, b))
        else:
            tmp = rnn_step_backward(dh[:, 0, :] + dh0, (x[:, 0, :], h0, Wx, Wh, b))
        dx[:, i, :] = tmp[0]
        dh0 = tmp[1]
        dWx += tmp[2]
        dWh += tmp[3]
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    da_i = i * (1 - i) * g * dnext_c
    da_f = f * (1 - f) * prev_c * dnext_c
    da_o = o * (1 - o) * np.tanh(next_c) * dnext_h
    da_g = (1 - g ** 2) * i * dnext_c
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(range_num):
        h0, c0, _ = lstm_step_forward(x[:, i, :], h0, c0, Wx, Wh, b)
        h[:, i, :] = h0
        c[:, i, :] = c0
        
    cache = (x, h, Wx, Wh, b, tmp, c)
...
def lstm_backward(dh, cache):
    ...
    for i in range(range_num, -1, -1):
        if i:
            tmp = lstm_step_backward(dh[:, i, :] + dh0, dc, (x[:, i, :], h[:, i - 1, :],  c[:, i - 1, :], Wx, Wh, b))
        else:
            tmp = lstm_step_backward(dh[:, 0, :] + dh0, dc, (x[:, 0, :], h0, 0, Wx, Wh, b))
        dx[:, i, :] = tmp[0]
        dh0 = tmp[1]
        dc = tmp[2]
        dWx += tmp[3]
