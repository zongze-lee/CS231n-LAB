...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss_real = bce_loss(logits_real, torch.ones(logits_real.shape[0]).type(dtype))
    loss_fake = bce_loss(logits_fake, torch.zeros(logits_fake.shape[0]).type(dtype))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss_real = scores_real.squeeze() - torch.ones(scores_real.shape[0]).type(dtype)
    loss_fake = scores_fake.squeeze() - torch.zeros(scores_fake.shape[0]).type(dtype)
    
    loss = torch.mean(1 / 2 * ((loss_real) ** 2 + (loss_fake) ** 2))
...
def ls_generator_loss(scores_fake):
    ...
    loss = torch.mean(1 / 2 * (scores_fake - torch.ones(scores_fake.shape[0]).type(dtype)) ** 2)
